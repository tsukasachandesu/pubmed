...
- `src/pubmed/`
  ...
  - `browser/`: `botasaurus` 構成をまとめる層。
    - `factory.py`: `botasaurus` の `browser_session` を初期化（Stealth/Proxy/ローテーション UA）。並列数や headless の切替を設定値からロード。
    - `tasks.py`: Sci-Hub/レート制限を受けた OA サイト用のブラウザタスクを定義し、純 HTTP で失敗した場合のみ実行。
  - `utils/`
    - `http.py`: `httpx.AsyncClient` を使ったセッション生成、UA/プロキシ/リトライポリシー（指数バックオフ）。`botasaurus` ベースのヘッダ/フットプリントを共有化し、Crawler 構成を集中管理。
  ...
  - `downloaders/`
    ...
    - `scihub.py`: 複数ミラーのローテーション、フォーム送信/iframe 解析、Captcha/HTML 失敗検知。`botasaurus` の Headless/Stealth ブラウザセッションを利用し、Captcha 出現時は `browser` スタックへ自動フェールオーバー。`--enable-scihub` フラグ必須。
    - `fallback.py`: 優先度付きの戦略（PMC → Unpaywall → Sci-Hub）。例外を握りつぶさず理由を Download レコードへ記録。
  - `workflow.py`: PMID リストを入力にメタデータ取得 + 全文ダウンロードを非同期キュー（`asyncio.gather` + セマフォ）で実施。再試行ポリシーを集中管理。`botasaurus` のキュー/優先度設定を読み取り、ブラウザ実行が必要なジョブを分離。
...
- **共通ポリシー**
  ...
  - UA ローテーション + オプションのプロキシ（botasaurus/scihub-cli 由来の戦略を反映）。`httpx` クライアントと `botasaurus` ブラウザで共通の UA/Proxy 設定を共有し、切り替えを容易化。
  - コンテンツ検証: PDF MIME/先頭バイトチェック、サイズ下限チェック。
  - 保存時はテンポラリ → チェックサム計算 → アトミック move。`Download.status`/`error`/`attempted_at` を更新。
  - `botasaurus` を用いたブラウザフォールバック: HTTP 失敗・Captcha 発生時はブラウザジョブに切り替え、Cookie 継承とセッション再利用で検出回避を強化。
